function ia = getGeneSet(geneset, data, HVGenes)
...
switch geneset   
    ...
    case 'merged'
        ...
        %lm22 = load('data/lm22.mat');
        %Immunostates = load('data/Immunostates.mat');
        %IRIS1 = load('data/IRIS1.mat');
        %IRIS2 = load('data/IRIS2.mat');
        markers = load('data/marker_genes1.mat');
        
        selected_genes=markers.genes;
        %table_s3 = load('data/table_s3.mat');
        %table_s12 = load('data/table_s12.mat');
       % selected_genes = unique(union(markers.genes, lm22.genes));
        selected_genes = unique(union(selected_genes, HVGenes));
     
        %selected_genes = unique(union(selected_genes, IRIS2.genes));
